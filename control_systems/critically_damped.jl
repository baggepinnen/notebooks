...
# ╔═╡ 9f5d2eea-50be-4de4-94a1-7fab8006399a
using Symbolics, ControlSystems, Plots, PlutoUI

# ╔═╡ 590e213c-e081-4045-b390-d24c6ef5cf74
md"""
# Critically damped
...
# ╔═╡ 5d745644-de82-4db8-802c-0df758f37a58
md"""
## Symbolic example
The following shows how to chose the PI parameters to place the poles with frequency ``\omega`` and damping ``\zeta``.
"""

# ╔═╡ 3f3821fc-1b1b-46c4-af40-649e2a8f8776
let
	@variables p l ω
	ζ = 1
	P = tf(1, [p, -l])
	pi_params, C = placePI(P, ω, ζ, form=:parallel)
end

# ╔═╡ 9aca5b08-8ca0-4b7b-abb4-60acc85d4f43
md"""
## Numerical example
We now turn to a numerical example. The desired bandwidth of the closed-loop system can be controlled by a slider. We can change ``l`` to be positive or negative.
"""

# ╔═╡ cbaa532f-e886-4c95-8499-04860ddd50a6
md"The pole-zero map indicates that we have a cancellation if we choose ``\omega = 1``, but not otherwise."

# ╔═╡ 27f823c8-ced5-4321-b318-16d691b0f05e
begin
	p = 1
	l = 1
	ζ = 1
	ws = @bind ω Slider(0.1:0.1:3, show_value=true, default=1)
	md"ω = $ws"
end

# ╔═╡ d95ec01a-fa37-455e-8b87-4b428b3b9267
P = tf(1, [p, l])

# ╔═╡ 9d1f3abf-7c00-47af-843e-743c6ada2fb2
pi_params, C = placePI(P, ω, ζ, form=:parallel)

# ╔═╡ b50dd9b9-bef6-492c-9c98-68907e1c75bc
T = minreal(feedback(P*C)) # closed-loop

# ╔═╡ 9701102c-2b59-4f23-97da-c43f7f27a2b5
begin
	F = tf(1, numvec(T)[])
	F = F/dcgain(F) # prefilter
end

# ╔═╡ 501e1b40-95e5-484b-b093-47da7b9c4385
pzmap(T, xlims=(-2,0.1), ylims=(-0.5, 0.5))

# ╔═╡ 0aaf4b8e-8f6f-4392-b7f6-32d30087c1a0
md"The step response and complimentary sensitivity function indicate that there will be an overshoot if we try to make the system significantly faster using feedback, i.e., if we make ``\omega > 1``."

# ╔═╡ a6f20e6d-f23c-4a73-9e0d-ddc2801b239d
plot(
    plot([
        step(T)
        step(T*F)
    ], lab=["Closed loop" "Prefiltered"]),
    bodeplot(T, plotphase=false),
)

# ╔═╡ f7a864e0-0903-4984-a605-5fff28f38337
nyquistplot(P*C, xlims=(-3,1), ylims=(-10, 0))

# ╔═╡ 7859ecf2-51c0-48fb-9a04-50f4f85f1646
md"ω = $ws"
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "2f8e98bed34b59af33b2a6c44a6a9d347100d8a6"
...
git-tree-sha1 = "6f134dc6ac39afe8edd6592750f2c28c78e5ba75"
