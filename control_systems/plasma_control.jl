...
# ╔═╡ 15e35878-ea5c-11ed-13bc-9debf5fe6b10
using ModelingToolkit, DifferentialEquations, MethodOfLines, ControlSystems, Plots, ControlSystemsMTK, Interpolations

# ╔═╡ 7ce96545-c98f-4e63-85b5-72aebdb4b05a
md"""
# PDE control
This notebook solves Poisson-style equation and adds an LQR controller to control the solution in the center of the domain.

We finish off by simulating the closed-loop system with a sinusoidal disturbance.
"""

# ╔═╡ b8f58fec-4eb0-4176-aed8-30199d64c3f6
md"""
## Outline
...
# ╔═╡ 25ee0d9e-e817-4c9f-a6c3-803db025403f
md"""
## Define PDE system

The model comes from this discource post
[Solving Grad-Shafranov Equation with ModelingToolkit by Luke Stagner](https://discourse.julialang.org/t/solving-grad-shafranov-equation-with-modelingtoolkit/60513/11?u=baggepinnen). We have modified the model to add a control input and a disturbance input. Both of these inputs act through *"influence functions"* which are chosen to be Gaussian blobs with a very limited support.
"""

# ╔═╡ 4181b668-d99a-44e3-bb66-76041ba98eb7
begin
	@parameters x y t
	@variables u(..) a(t)=0
...
# ╔═╡ fa95c98f-d87f-44bb-a0f8-aed148669d4f
md"""
...
# ╔═╡ f9ce236d-fe96-4440-a3c0-3314d1699e2f
# ╠═╡ show_logs = false
begin
...
# ╔═╡ c24e322c-7598-4cd1-9f3f-4c102f2ed70b
md"""
...
# ╔═╡ ca7cef4a-ed12-43a5-b096-0a4175b08549
# ╠═╡ show_logs = false
begin
...
# ╔═╡ 30765728-4f28-41f5-a98c-5b68d7921874
md"The linear system has a large number of state variables:"

# ╔═╡ 2648902d-eb50-4567-953a-ac7127f1a181
lsys.nx

# ╔═╡ 75c7d9b3-46e3-4bac-9446-97226ce579d6
md"In a practical scenario where we deploy the controller using an observer, or if we implement an MPC controller etc. ee could perform model-order reduction to reduce the computational complexity:"

# ╔═╡ 1eccf367-b7d3-4879-8422-2b4db4d977b5
rsys, _ = baltrunc(lsys) # Perform model reduction

# ╔═╡ d4e0e729-c7c4-4218-8591-775fe6b252a3
md"""
...
# ╔═╡ d4936ab0-c96c-40d4-b6e9-6c80ba6ffb4b
md"""
...
# ╔═╡ 79035ff3-f9b9-4171-8685-b7727536a876
begin
	C = lsys.C
	Q = C'*C
	R = 0.00000001
...
# ╔═╡ 677b6e9b-efcf-4a45-9fa4-e9be6b97c6a3
md"How does the LQR controller behave?"

# ╔═╡ c1ebd9de-87f8-42d1-a742-965c403c0247
begin
	inp(x, t) = -L*x
...
# ╔═╡ cfe3a3db-0ad9-47e6-b25b-f9c65f790d9d
md"""
...
# ╔═╡ acf339f2-74ad-4e1c-a2a5-710055da63ee
begin
	Lmat = zeros(nd, nd)
...
# ╔═╡ a7c40e8f-d3cf-47b1-ac84-11091d8657e1
md"""
...
# ╔═╡ db071338-2c88-4d91-8265-59f2cd301c5b
md"""
...
# ╔═╡ 67265fa4-5376-47da-ba49-5f57029d7634
# ╠═╡ show_logs = false
begin
...
# ╔═╡ c25dd9ec-51d2-4e83-a87e-78d6ff6b7ce1
md"""
## Visualization
...
# ╔═╡ 81440076-5140-44ff-9c54-db5e3bc13acb
begin
	...
	showsol(sol_control) # This takes a while
end

# ╔═╡ 4d89ceeb-1f16-4abe-bb60-67d5c991c979
md"""
**Without control**, notice how the center fluctuates due to the disturbance:
"""

# ╔═╡ f832632b-5afd-4b31-b3bc-b6b8212aa3c6
showsol(sol_nocontrol)

# ╔═╡ 3e55652e-9fce-4576-8ee1-72e1989a2c36
md"""
Below, we show the solutiuon for a number of cells in the center of the domain:
"""

# ╔═╡ d9b19193-7769-4b03-aa74-616943af2137
begin
	yinds = (-1:1) .+ nd ÷ 2
	xind = nd ÷ 2
	ref = M_ss[xind, yinds]
...
# ╔═╡ 6b4ede3f-101c-404a-8d3f-c51c8a2cc7c6
md"""
The controller reduced the variation, does this mean that we will have infinite clean energy for everyone for the rest of our time on this planet? Probably not, but this was a fun control-systems example with a PDE system model.

## Caveats
- We used a toy model.
- We assumed that we could actuate the center of the domain which is most likely unrealistic in a plasma-control scenario. We also had a single actuator only.
- We assumed that full state feedback was possible, in practice, we would need a state observer.
- We performed no robustness analysis, neither did we inspect the control signal to verify that we are respecting actuator limitations etc., we should obviously do this before claiming that we have slayed the beast.
"""
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "3ae7599e65e47c0e84235acd2e0a8009694c0cc1"
...
git-tree-sha1 = "9649d2372f03201dff26b5542730ab37c457cbe1"
...
git-tree-sha1 = "14c301271b3f44df5ca786687b7d3146c988b1c8"
...
git-tree-sha1 = "157d8a1a3784584dfec3bafeb054e5df3c5dcb43"
...
git-tree-sha1 = "35639c616c18041fd0394a1d9ecfaeca0490254e"
...
git-tree-sha1 = "3cdc75132aa2b043de80d3b83bbf242630bae1cb"
