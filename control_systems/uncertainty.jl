...
# ╔═╡ 6b8d6584-c95f-11ed-1252-6bd63e62d1a7
using ControlSystemsBase, MonteCarloMeasurements, Plots

# ╔═╡ 1b56eaa2-196b-4f46-b948-7e5ddf2e3ff4
using RobustAndOptimalControl

# ╔═╡ bf895955-a0df-4dbf-a6b4-94cb516882d4
begin
...
# ╔═╡ 482ce1b2-9c11-42e2-a1f9-a12136ab4d54
begin
	using ControlSystemsMTK
	@named Wsys = ODESystem(makeweight(0.1, 1, 0.9))
	@parameters Δ=(-1..1) # The uncertain element is between -1 and 1
	@variables y(t)=0
	...
	@named uncertain_sys = ODESystem(connections, systems=[sys, Wsys])
	initial_condition = [sys.x => 0.0, sys.v => 0.0, sys.k => 10, sys.d => 1]
	probu = ODEProblem(structural_simplify(uncertain_sys), initial_condition, (0.0, 10.0))
	solu = solve(probu, Tsit5())
	plot(solu, ri=false, N=500, idxs=[sys.x, sys.v, y], layout=2, sp=[1 2 1], l=[3 3 1]) # ribbon = false and plot 500 sample trajectories
end

# ╔═╡ ba53382b-9e69-41e4-9372-c29237f576e8
using PlutoUI

# ╔═╡ 5c6e855a-03fc-4d0f-998f-93e18e086624
md"""
# Dynamical systems with uncertainty

This notebook will explore a few simple ways to model uncertainty in control systems. For additional resources, see [RobustAndOptimalControl.jl](https://juliacontrol.github.io/RobustAndOptimalControl.jl/dev/).
"""

# ╔═╡ 1d371994-ca24-45a0-8e01-af2ce8581b7a
md"""
## Uncertain parameters

The perhaps simplest and most intuitive way of modeling uncertainty is in the form of *parametric uncertainty*, i.e., you are not quite sure about the value of a parameter. We are commonly interested in two different forms of parametric uncertainty
1. The parameter is known to be distributed accoring to some probability distribution. This is common when the parameter is derived from statistically motivated parameter estimation.
2. The parameter is known to lie in an interval.

Below, we show how to created parameters that
1. Follow a normal distribution with specified mean and standard deviation
2. Follow a uniform distribution
"""

# ╔═╡ 911db2d6-7f96-4945-87cd-fa965020af69
ω = 1 ± 0.1 # Create an uncertain Gaussian parameter

# ╔═╡ 8c569959-9656-48ce-80eb-92aa0287ce51
ζ = 0.3..0.4 # Create an uncertain uniform parameter

# ╔═╡ 16f1f175-64d4-46cd-b1ed-7dc93177c245
md"The parameter distributions are represeted as collections of samples, here called *particles*. They behave just as if they were standard numbers, like `Float64`, that is, you can perform standard computations with them:"

# ╔═╡ 1e7930f7-3c05-44c3-a97d-2effb4d711fa
2ω^2 + sin(ζ)

# ╔═╡ aa807681-fb24-4518-a22b-ef275277182b
md"You can also plot them:"

# ╔═╡ 9d0f9a0a-db31-4de5-a446-568ecf79a477
plot(2ω^2 + sin(ζ), title="I'm not quite sure about \$2ω^2 + \\sin(ζ)\$ but please accept my paper anyway because I'm at least honest", titlefontsize=7)

# ╔═╡ 8337cfa4-74b6-47d5-a70b-ec7f46efef60
md"""
## Uncertain dynamical systems

We can use such uncertain parameters to build models of dynamical systems. Below, we construct a transfer-function model and perform some common control-systems analysis on it
"""

# ╔═╡ a97ae8b9-60fb-4a3b-9de5-2dc9d09269f0
G = tf(ω^2, [1, 2ζ*ω, ω^2])

# ╔═╡ f76dc5d7-e2a8-43f4-ab76-5c2ad12fdb3b
dcgain(G)

# ╔═╡ 0b64f7c0-c1e3-4dbb-bba7-15851bcfaf7d
abs(freqresp(G, 1)[])

# ╔═╡ cd2d73f9-6401-47bf-95c6-a66c2f74d8ac
begin
	w = exp10.(-2:0.02:2)
	bodeplot(G, w, legend=:bottomleft)
end

# ╔═╡ 1981bca0-ba19-4388-902f-6ec6bfe61330
plot(step(G, 0:0.1:20), ri=false, N=2000, alpha=0.02)

# ╔═╡ 5d158f5c-f3a9-4aa1-8f68-8349d8bdc6ce
md"Note how all the analysis code above looks identical to how it would look if we had used a model without uncertain parameters, save for the `plot` command which also indicated how many samples to plot `N = 2000`. "

# ╔═╡ f80d73ca-f213-4f34-adba-6a3495d3fbdf
md"""
### Uncertain time delay
The following creates a model of a dynamical system with an uncertain output delay. The delay is known to be exactly 1,2,3 or 4 milliseconds. We call the constructor of `Particles` directly in this case, providing the vector of 4 possible delay values.
"""

# ╔═╡ 25cf4777-0daf-4c48-b203-e0387a8c88a8
begin
	...
	w2 = exp10.(-1:0.1:4)
	plot(
	     bodeplot(P, exp10.(-1:0.001:3), legend=false, ri=false, alpha=1),
	     nyquistplot(P*K, w2, points=true, xlims=(-1.5, 2.2), ylims=(-3, 1), Ms_circles=[2], alpha=1, m=(0.5))
	)
end

# ╔═╡ c82f4e1c-5d75-47cb-a9dd-da9819d21a01
md"When we plotted the Nyquist plot, we indicated that we wanted to see the samples as `points = true`, which makes the plot easier on the eyes."

# ╔═╡ 51426e79-6a37-45ff-b2f2-645713af1a6b
md"""
### Convert uncertain system models to vectors of models

The uncertain system model is represented as a standard model, but with coefficients of the special type `Particles` instead of, say, `Float64`. We can convert such a representation of an uncertain model to a vector of models with `Float64` coefficients instead, for this, we load `RobustAndOptimalControl.jl` which contains a lot of extra utilities to work with uncertain models.
"""

# ╔═╡ ebc0840a-1b32-43dd-a03d-6063851c4d9a
Ps = RobustAndOptimalControl.sys_from_particles(P) # Creates a vector of standard models

# ╔═╡ 7aebd5b7-36b0-4c0f-975a-3699f8d6f2c6
md"When we plot this vector of models, we get a similar plot to the one above, but this time we get different colors since the models are all treated as separate models, rather than a single uncertain model."

# ╔═╡ 2031fb43-9126-40d5-9e0e-66adce94ef7e
bodeplot(Ps)

# ╔═╡ 6db14e4a-5ff2-4bdf-8a28-60102186c40c
md"""
### Convert vector of models to uncertain model
We can go in the other direction as well, create a model with uncertain coefficients from a vector of models:
"""

# ╔═╡ 07502586-8b7c-418c-a26e-d203f48c0cc8
Gs = RobustAndOptimalControl.sys_from_particles(ss(G))

# ╔═╡ b9c60434-0f4e-4f7f-87b6-5b6c4f84738b
G2 = RobustAndOptimalControl.ss2particles(Gs)

# ╔═╡ e7de9f81-bec0-438a-beeb-1c90b4a26ba6
bodeplot([G, G2], w)

# ╔═╡ 67e9c29b-30fa-45e2-9634-6df7b8340454
md"""
### Modeling unstructured uncertainty
Parametric uncertainty is commonly occuring type of uncertainty, but how do we model things like
> My model is accurate for low frequencies but *I really have no clue above 5rad/s*
?

We can use *unstructured multiplicative uncertainty*
```
                  ┌────┐          ┌────┐
                ┌►│ WΔ ├┐      ┌─►│ WΔ ├──┐
                │ └────┘│      │  └────┘  │
...
└───────────────────────┘        │ I+PC │
                                 └──────┘
```
Where ``||\Delta|| < 1`` and ``W(s)`` is a frequency-dependent weight. This form of uncertainty model is common in the field of *robust control*.
"""

# ╔═╡ f539b760-f474-4fb5-8b8a-9d254a66b795
W = makeweight(0.1, 5, 2)

# ╔═╡ daaada5d-464d-4fc6-914a-5f8c5073c51c
md"Visualize the weight function:"

# ╔═╡ 16cd0c1a-7ac0-4d1b-81e7-f0fb5e24ed78
bodeplot(W, plotphase=false, lab="Weight \$W(s)\$", ylabel="Relative uncertainty"); vline!([5], lab="Not sure beyond here", legend=:topleft, l=(:black, :dash))

# ╔═╡ b6e5e55e-6157-4592-b47b-02d28276e354
md"To construct the uncertain plant model, we start with a nominal system model ``P_0(s)``"

# ╔═╡ 1c38094c-c07c-464c-b178-8b2cbb574ffb
P0 = tf(1, [1, 2, 1])

# ╔═╡ 00605378-4801-4d2d-af84-3a362e201c02
md"And then multiply with ``(1 + W δ)`` to obtain the uncertain system model ``P_u(s)``:"

# ╔═╡ 26a45144-cd4d-47d1-ad6a-5c4232d8bb2d
Pu = P0 * (1 + W*δc(500))

# ╔═╡ 3932824d-581a-4fc3-84ea-4a3972378cf6
md"Above, we called `δc(500)` to indeicate that we wanted a complex uncertain element represented by 500 samples. RobustAndOptimalControl has tools to work with perturbations that are represented symbolically instead of using samples, but in this notebook we stay with the sampled representation.

The next step is to form a controller `C` and analyze the closed loop:"

# ╔═╡ f379e48f-79c2-4e42-a9a9-f515742af2a9
C = pid(1,1, Tf=0.01); # A simple controller

# ╔═╡ e0204f1d-4848-4de2-b021-590ca2aefe7a
Gcl = feedback(Pu*C); # Close the loop

# ╔═╡ e604456d-89d4-42a9-91a3-ee2824954343
bodeplot(Gcl, w, legend=:bottomleft, title="Uncertain Closed-loop \$G_{cl}(s)\$"); ylims!((1e-4, 3), sp=1)

# ╔═╡ dde42ed2-8691-413a-ad37-c1fc417f8a41
md"""
We can approximate the uncertain system with circles in the Nyquist plane:
"""

# ╔═╡ b401ed35-bed9-45c7-addd-320998092e57
begin
	nyquistplot(P0*C, w, lab="Nominal", Ms_circles=[1.5])
	centers, radii = fit_complex_perturbations(Pu*C, w, relative=false)
	nyquistcircles!(w, centers, radii, ylims=(-3,0.5), xlims=(-1.5, 0.1), c=2)
end

# ╔═╡ 5124d130-67a3-476d-8bdc-fa52d400101f
md"""
## Parametric uncertainty in ModelingToolkit

We can use the same approach with uncertain parameters in ModelingToolkit models, here's an uncertain mass-spring-damper

(if you get an error in the cell below after launching the notebook, re-run the cell)
"""

# ╔═╡ 67810bea-21c7-47f2-85c6-2858c119cfce
md"""
We simulate with initial condition ``x = 0.0 ± 0.1`` and spring  stiffness ``k = 10 ± 1``
"""

# ╔═╡ de5713bd-013c-42ee-af97-e8f45c1df67b
begin
	prob = ODEProblem(sys, [x => 0.0 ± 0.1, v => 0.0, k => 10 ± 1, d => 1], (0.0, 10.0))
	sol = solve(prob, Tsit5())
	plot(sol, ri=false, N=1000) # ribbon = false and plot 1000 sample trajectories
end

# ╔═╡ 8489e69a-ece8-4279-8918-4120006d2443
md"""
## Unstructured uncertainty in MTK
Here, we use the same approach as above, and connect a sampled uncertain element in series with a weighting system ``W_{sys}``. The package `ControlSystemsMTK` is loaded so that we convert the weight to an `ODESystem` from MTK
"""

# ╔═╡ e1dd11a9-a8d2-4d12-8296-3a8e3b7d6c9f
md"""
Be careful with how the result is interpreted here, we connected the unstructured uncertainty at the output, so the internal states are not affected in this simulation.
"""

# ╔═╡ a972f3f3-f5b9-49b6-b28a-c54ad5ef7371
md"""
## Concluding remarks

- It's easy to model uncertain systems, either as parametric uncertainty or as unstructured uncertainty.

- We can simulate uncertain systems and plot Bode and Nyquist plots etc.

- Robust control design for uncertain systems is a future notebook topic 😃
"""

# ╔═╡ aa57c89c-3e37-4255-a788-351b5b6c9f8e
...
ControlSystemsMTK = "~0.1.11"
ModelingToolkit = "~8.48.1"
MonteCarloMeasurements = "~1.1.3"
OrdinaryDiffEq = "~6.49.3"
Plots = "~1.38.6"
PlutoUI = "~0.7.50"
RobustAndOptimalControl = "~0.4.21"
"""
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "cff6903d778828fcaab7576740b8f1e9e695afdd"
...
git-tree-sha1 = "2cd514469fb145329b9b0e933989bd4259e654e7"
...
git-tree-sha1 = "d9d8db973687145db5537ce8877357691db92507"
...
git-tree-sha1 = "bdec8591da9a250db5643ae65259f69eb8665d23"
...
git-tree-sha1 = "63ebdaa270aaa6c2f7a8984b2a585e4cdbaa1300"
