...
# ╔═╡ 8d550920-5865-11ee-0937-db9ec4cf9a1b
using ControlSystemIdentification, ControlSystemsBase, Plots, Statistics, LinearAlgebra, Random, LowLevelParticleFilters, StaticArrays, SeeToDee

# ╔═╡ 506a2691-f45a-4a21-908f-4b79f5d1c8f3
gr(); theme(:dark)

# ╔═╡ 2ec00165-1e79-49ca-9e11-bff48e903a57
md"""
# Gray-box identification of linear and nonlinear models

*Known structure, unknown parameters*

A linear statespace model
```math
\begin{align}
\dot x &= Ax + Bu + w \\
y &= Cx + Du + e
...
## Predictor?
For linear systems, a linear observer is common (Kalman filter and friends)
```math
\begin{align}
\dot x &= Ax + Bu + K\varepsilon \\
y &= Cx + Du + \varepsilon
\end{align}
```
...
# ╔═╡ a59dc731-52fb-4000-932f-817dad7978de
md"""
# A nonlinear system
...
# ╔═╡ 43508cd3-38ef-4d9f-a22e-3f012bd96bc5
begin
	function quadtank(h, u, p, t) # Continuous-time dynamics
...
# ╔═╡ 14cdbc41-129e-4bc3-b41b-405f2ec910e5
begin
	nx = 4
	ny = 2
	nu = 2
...
# ╔═╡ 0318597d-fcdc-4d5a-af25-8834611db547
begin
...
# ╔═╡ 68b2d5d7-0b90-4568-9717-cfb6510f0442
d = iddata(Y, U, Ts)

# ╔═╡ 99d55071-35fd-440f-9f20-7f6feb83d07a
begin
	x0_guess = [2.5, 1.5, 1, 2] # Guess for the initial condition
	p_guess = [1.4, 1.4, 5.1, 0.25] # Initial guess for the parameters
		
	R1 = Diagonal([0.1, 0.1, 0.1, 0.1]) # Covariance of w
	R2 = 100*Diagonal(0.5^2 * ones(ny)) # Covariance of e
end

# ╔═╡ 8b953e0b-02c9-4ec7-a1ac-ea32e1afe214
...
# ╔═╡ c6ab950d-f96b-4b1d-904d-a97db919ce86
begin
	simplot(model, d, layout=2)
	#predplot!(model, d, ploty=false)
	# Simulate the system with the initial guess parameters for comparison
	x_guess = LowLevelParticleFilters.rollout(discrete_dynamics, x0_guess, u, p_guess)[1:end-1]
	y_guess = measurement.(x_guess, u, 0, 0)
	Y_guess = reduce(hcat, y_guess)'
	plot!(Y_guess, lab="Initial guess")
end

# ╔═╡ 34608f7f-f8bc-4563-a8fb-81d78433102f
begin
	plot(d, layout=4, lab="Data")
	plot!(Y_guess, lab="Simulation with initial parameters", legend=:outertop)
end

# ╔═╡ 5433fd3d-3c0a-4157-aa00-47aa379a3e8b
residualplot(model, d)

# ╔═╡ 82cbded3-16f6-41a1-936d-34f557236627
[p_true p_guess model.p]

# ╔═╡ 0b763f22-961d-4644-abab-5e7a9d8247c5
md"""
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "974b96e19545dc6acc1b9ee88ff766cb849c5766"
