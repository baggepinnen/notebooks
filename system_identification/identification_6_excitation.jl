...
# ╔═╡ 2e57efa1-ab31-4946-a147-893e19cf6669
using ControlSystemIdentification, ControlSystems, Plots, PlutoUI

# ╔═╡ 0da4c8c6-c37b-48d2-bf73-7331836d7ab9
theme(:dark)

# ╔═╡ 22b6b74c-17b7-4044-8342-c4c95ce5b251
PlutoUI.TableOfContents()

# ╔═╡ 7ed1e1da-a900-4f0b-9c4c-4a6008ae6519
begin
	P1 = tf(1.0, [1, 1])
	P2 = tf(1.0, [0.2, 0.1, 1])
	lab = ["\$P_1(s)\$" "\$P_2(s)\$"]
...
# ╔═╡ 570adc9c-fc6a-4cf6-aeae-6fafd0dce7e6
begin
	...
	Gnl = feedback(P1*satC) # closed loop with saturation
	s1 = step(Gnl, 100)
	s2 = step(Gnl*2, 100)
	plot(s1, lab = "Step size 1")
	plot!(s2, lab = "Step size 2")
	plot!(s1.t, 2 * s1.y[:], lab = "2 × Step size 1", alpha=0.5, title="Step responses of various amplitude")
end

# ╔═╡ 579d6891-af80-4148-85e8-500efa62bbe2
html"<button onclick='present()'>Present</button>"

# ╔═╡ 6eadb51e-1ae7-4c18-8629-60f97d38be26
md"""
# Performing experiments

When performing system identificaiton, it's important that our data is *informative* or *exciting*.
...
- Theoretical concept
- Roughly equivalent to rank of correlation matrix of excitation signal
- Indicates how complicated models a particular signal allows us to identify

## For linear time-invariant systems:
- The **impulse response** is a complete characterization
- The **frequency-response** function is also a complete characterization (Laplace transform of impulse response)

## Impulse response experiments
- ``u(t) = δ(t)`` 
- Flat spectrum, PE of any order 👍
- Theoretical impulses are **impossible to realize** 👎
- Practical impulses may still **damage system** 👎
- Very **high sensitivity** to few samples (repeated experiments may be required) 👎
- Common in acoustics and structural-resonance modeling (hammer impact)
"""

# ╔═╡ 465a2036-25e6-4e15-b7bf-cd618df8174f
plot(impulse.(sys, 25); title="Impulse responses", lab)

# ╔═╡ 5712362e-5ee0-4edd-b270-1fd8016569cf
md"""
## Frequency-response experiments
- Measure one point on FRF at a time ``G(i\omega) = K e^{i \Delta\phi}``
- Multiple sines with different frequency and phase (PE of order ``2n``)
- Chirp

**About amplitude**:

Linear systems don't care about the input amplitude, but
- SNR typically improves with larger amplitude 👍
- Excitation of nonlinearities typically also increase... 👎
"""

# ╔═╡ 9e1006e7-0ff7-4d9f-a690-b9fcaf592420
begin
	ω = exp10.(-2:0.1:2)
	bodeplot(sys, ω; lab=permutedims(repeat(lab[:], inner=2)), m=:circle)
end

# ╔═╡ 21bce443-41ac-4e24-8018-29159e214d2b
function chirp(Ts, f0, f1, Tf)
...
# ╔═╡ 08cc1361-9e8c-4e41-99bf-bd3c38021639
md"""
Unexpected output frequencies are signs of nonlinearity
"""

# ╔═╡ 71ed43d9-119c-46ad-b20b-12c2c56c7ec1
md"""
With a chirp experiment, you can "see the frequency response" in real time:
"""

# ╔═╡ 7e79aa3e-4399-437a-9c8c-8e7fc0315ea6
md"""
## Random signals
- White noise: flat spectrum, PE of any order 👍
- Colored noise (allows frequency selection)
- Random binary sequences: PE of any order, lowest possible "crest factor" (large power with small amplitude) 👍
"""

# ╔═╡ 3b7a0e25-79df-428b-bd81-3dfea415b4df
begin
	u = randn(1, 200)
	t = range(0, step=0.1, length=length(u))
	plot(
		plot([lsim(P, u, t) for P in sys], plotu=true, plot_title="Gaussian"),
		plot([lsim(P, sign.(u), t) for P in sys], plotu=true, plot_title="Binary"),
	)
end

# ╔═╡ 55b3e685-4063-4665-92a2-9c5e96d8ebcc
md"""
We can look at the spectra of input and output
- Spectrum is flat for white noise
"""

# ╔═╡ 9a240738-d3cb-4205-8a27-8c2bc2987d9d
let
	u = randn(1, 2000)
	t = range(0, step=0.1, length=length(u))
	plot(
		welchplot(iddata(lsim(P1, u, t)), title=lab[1]),
		welchplot(iddata(lsim(P2, u, t)), title=lab[2]),
		legend = :bottomleft,
	)
end

# ╔═╡ 806634b2-c1ce-44ae-9d4f-234142ce850a
md"""
## Step-response experiements
- Dominated by low frequencies (``1/s``) 👍/👎
- PE of order 1 (very poor excitation) 👎
- Easy to perform 👍
- Simple model can be fit by eye 📈👀 👍
"""

# ╔═╡ aa110ae0-84d1-41c9-a4b6-455d993c4b6c
plot(step.(sys, 25); lab, title="Step-response experiment", plotu=true)

# ╔═╡ 2becbb5f-8646-4494-81df-708627db26d5
md"""
## Closed-loop data
Data collected in closed loop has **correlation** between noise and input and between output and input
- Many methods are **biased** in this setting 👎
- PEM is unbiased 👍
- Break correlation and introduce excitation by
    - Modifying references ``r``
    - Add input disturbance ``d``
...
\end{aligned}
```
cannot distinguish ``b`` from ``a``!

Remember the ARX(2,2) model from video 2:

```math
\mathbf{y}_{future} = \Phi \mathbf{w}
```
```math
\mathbf{w} = [a_0\; a_1\; b_0\; b_1]^T
```
...
\end{bmatrix} = [u = ky] = \begin{bmatrix}
y_2 & y_1 & ky_2 & ky_1 \\
y_3 & y_2 & ky_3 & ky_2 \\
y_4 & y_3 & ky_4 & ky_3 \\
\vdots & \vdots & \vdots & \vdots
\end{bmatrix}
```
``\Phi`` loses rank and we cannot form the estimate
```math
\mathbf{w} = (\Phi^T\Phi)^{-1} \Phi^T \mathbf{y}_{future}
```

#### With excitation through ``r``
```math
\begin{aligned}
y^+ &= ay + bu \\
[u &= k(r - y)] \\
...
# ╔═╡ da46236b-5417-467c-870d-433841647c37
md"""
# Nonlinearities
- Linear systems don't care about **amplitude** (except for noise properties)
- Nonlinear systems do, detect nonlinearity by performing the same experiment
    - With different amplitudes
    - Forward and reverse (e.g., chirp)
- Excitation for nonlinear systems is a *much more nuanced* topic.
"""

# ╔═╡ da028bbc-39b8-4f59-8ebf-8c370a7b96dd
md"""
# Evaluating the identification data
- Spectral properties of input
- Magnitude-squared coherence ``κ²``
- Covariance matrix ( ≈ ``(\Phi^T\Phi)^{-1}``)
...
```
Always between 0 and 1. Close to 1 if ``v`` is small.
- Low noise
- Non nonlinearity

Example below, chirp excitation from ``ω = 0.5`` to ``ω = 10`` with periodic disturbance ``v = \sin(2t)``
"""

# ╔═╡ 7942ed45-5423-43de-bbbe-189878e765a6
md"""
## Data covariance matrix
"""

# ╔═╡ fea72984-c3f8-4a19-a820-4354a2641350
begin
	d = iddata(lsim(P1, [0 ones(1, 99)], range(0, step=0.1, length=100)))
	plot(d)
end

# ╔═╡ dfe04ffc-df10-42b6-9a71-70423eadb209
let
	using LinearAlgebra
	yf, Φ = getARXregressor(d, 2, 1)
	(; Σ = svdvals(Φ), rank = rank(Φ))
end

# ╔═╡ d39cd740-e726-461b-b7ec-38b77f5f3d17
let
	t = 0:0.1:500
	u = @. sin(t) + sin(2t) + sin(3t) + sin(5t)
	simulations = [iddata(lsim(P, u', t)) for P in sys]
	figdata = plot(simulations[1][1:300], plot_title="Multi-sine")
	plot!(simulations[2][1:300])
	plot(
		figdata,
		welchplot(simulations[2], title="Spectra for \$P_2(s)\$ experiment"),
	)
end

# ╔═╡ 202f59eb-2a14-4a38-8975-af0f8a03b400
fig1, fig2 = let
	Tf = 500
	Ts = 0.02
	t = 0:Ts:Tf
	u = chirp(Ts, 0.5/2π, 10/2π, Tf)
	simulations = [iddata(lsim(P, u', t)) for P in sys]
	figdata = plot(simulations[1][1:2end÷3], plot_title="Chirp")
	plot!(simulations[2][1:2end÷3], sp=[1 2], alpha=0.7)
	fig1 = plot(
		figdata,
		welchplot(simulations[2], title="Spectra for \$P_2(s)\$ experiment"),
	)
	fig2 = specplot(simulations[2])
	fig1, fig2
end;

# ╔═╡ 9f696542-5b90-488e-899d-8d83078734e9
fig1

# ╔═╡ 43d8c714-f361-4a07-abba-0eb0a1d37389
fig2

# ╔═╡ 3e7baa28-5444-4785-928b-e58321e62355
let
	Tf = 500
	Ts = 0.02
	t = 0:Ts:Tf
	u = chirp(Ts, 0.5/2π, 10/2π, Tf)
	d = iddata(lsim(P1, u', t))
	dnl = deepcopy(d)
	dnl.y .= dnl.y.^3
	cp = coherenceplot(d, lab="Linear")
	coherenceplot!(dnl, lab="Nonlinear")
	plot(
		specplot(dnl),
		cp,
	)
...
# ╔═╡ 75dbf5f7-efce-440d-964a-9821876a15df
	let
		Tf = 500
		Ts = 0.02
		t = 0:Ts:Tf
		u = chirp(Ts, 0.5/2π, 10/2π, Tf)
		res = lsim(P2, u', t)
		@gif for i in 1:200:length(t)
			plot(res.t[1:i], res.y[1:i])
		end
	end

# ╔═╡ 5e5cef52-7dbc-4ab9-aa3f-901e3336455e
let
	Tf = 500
	Ts = 0.02
	t = 0:Ts:Tf
	...
	res.y .+= sin.(2 .* res.t)'
	d = iddata(res)
	xt = Real[0.5, 2, 10, 100]
	figdata = plot(res, plot_title="Chirp")
	cp = coherenceplot(d, xticks=(xt, string.(xt)))
	plot(plot(
		figdata,
		cp,
	), welchplot(d, title="Spectra for \$P_2(s)\$ experiment"), layout=(2,1))
end

# ╔═╡ 05d32cfa-0182-49a9-a879-beb0a22a09ca
md"""
Correlation between the input and output, as well as autocorrelation in the input, can decrease the identifiability. The regressor matrix ``\Phi`` loses rank if some columns are linearly dependent on other columns. ``\frac{1}{N-1}(\Phi^T\Phi)^{-1}`` is sometimes called the covariance matrix of the data.
"""
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "230785799ca29220af2df7688c4b606a8a70c3cd"
...
git-tree-sha1 = "2aca06030445ffe58b88328051c3df538ca0ebd7"
