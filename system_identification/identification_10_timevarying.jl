...
# ╔═╡ b2c6088e-737e-4706-b879-b43ec413f1c3
begin
...
# ╔═╡ 79731c06-5939-11ee-29a3-2d059d1a6b37
md"""
# Estimation of time-varying parameters
What's the difference between a *parameter* and a *state variable*?
- Parameters are constant, states are evolving?
- Parameters are known, states are unknown?
- State variables are driven by input, parameters aren't?

If a parameter is both unknown and time varying, is it really a state variable?

## Estimating time varying *unknowns*
A *state estimator* or *observer* estiamtes the current state of a system given past inputs and measurements.

```math
\dot x(t) = f(x(t), u(t), p(t), t)
```

We can trivially make use of it to estimate ``p(t)`` by simply extending the state
```math
x_e(t) = \begin{bmatrix}x(t) \\ p(t)\end{bmatrix}
```

!!! note
	Observability and identifiability conditions are relavant!


If problem was previously linear in the unknowns, 
```math
\dot x = A x
```
it is now nonlinear.
...
# ╔═╡ f932e7af-ae09-4e96-a031-caa05e4e5a51
md"""
# Example: Quad tank
From [video](https://youtu.be/GKl8Tz9n2gs?si=xPkDoIzMeE5myvTC) and [notebook](https://juliahub.com/ui/Notebooks/fredrik-carlson2/system_identification/identification_9_graybox.jl) 9

![Quadtank](https://user-images.githubusercontent.com/3797491/166203096-40539c68-5657-4db3-bec6-f893286056e1.png)
...
# ╔═╡ 24ca212a-0236-443d-8760-3506f5710697
md"""
Dynamics with ``a_1`` changing during the simulation
"""

# ╔═╡ 8fcc6f46-cee2-4dc7-88b4-1b4ecf94f731
...
# ╔═╡ 2297ad13-fca2-44d4-9348-bbf90d130500
md"""
# Data
"""

# ╔═╡ 345abed7-5df0-43c2-bb2d-d8daa46e990b
md"""
# Dynamics of extended state
"""

# ╔═╡ 8f73af08-8211-4ed6-bcb1-af4783de7eff
md"""
...
# ╔═╡ e2ababd0-5f87-4829-b4e4-77518158b0fe
md"""
# Covariance estimates

Statistically motivated state estimators maintain a posterior probability distribution over the state.

The measured state variables have low variance, the others not:
"""

# ╔═╡ e6fa07a7-7c12-4c1f-a793-054641181b25
md"""
If the variance for some estimate blows up, you are suffering from identifiability problems!

# Concluding remarks
State estimation is useful for a lot of things
- Estimate / predict state
- Virtual sensing
- *Online* estimation of parameters
    - State *smoothing*  is useful in offline settings
- Change detection
- Estimation of unknown inputs
"""

# ╔═╡ e3b5c56a-1288-4e00-aab3-34193d226ac6
md"""
---
# Generate data
"""

# ╔═╡ 31aad581-6aba-4a09-971e-36b0ec95e9b4
theme(:juno); gr(dpi=300);

# ╔═╡ ef235bde-c96d-4c28-92ea-eb5b96841e26
begin
...
# ╔═╡ 367a86df-9ba0-4fd0-bede-de6a007adcfe
begin
	function quadtank_paramest(h, u, p, t)
	    ...
	    a1 = h[5] # the a1 parameter is a state variable
...
# ╔═╡ 665fe26f-c5f5-49dc-a624-0e0114e55b6d
begin
	measurement(x,u,p,t) = SA[x[1], x[2]]
	discrete_dynamics = SeeToDee.Rk4(quadtank, Ts)
end;

# ╔═╡ 880a2dbb-e9e3-449f-b66f-f246b4fd740f
begin
	Tperiod = 200
	t = 0:Ts:1000
...
# ╔═╡ bbdc7897-dc60-474d-8e9c-99f988ea8aac
	plot(
	    plot(reduce(hcat, x)', title="Tank levels", lab="h".*string.((1:4)')),
	    plot(reduce(hcat, u)', title="Inputs", lab="")
	)

# ╔═╡ 6cd50437-5919-4e7c-ab2e-57b5dce54dce
begin
	R1 = Diagonal([0.1, 0.1, 0.1, 0.1, 0.0001])
	R2 = Diagonal((1e-2)^2 * ones(ny))
	x0e = [2, 2, 3, 3, 0.02]
	
	kf = UnscentedKalmanFilter(discrete_dynamics_params, measurement, R1, R2, MvNormal(x0e, R1); ny, nu)
	
	sol = forward_trajectory(kf, u, y)
	plot(sol, plotx=false, plotxt=true, plotu=false, ploty=true, legend=:bottomright)
	plot!([0,500,500,1000], [0.03, 0.03, 0.06, 0.06], l=(:dash, :white), sp=5, lab="\$a_1\$")
end

# ╔═╡ b8afe13d-775a-4dc6-802a-dc8935f711b0
begin
	σ = reduce(hcat, [.√(diag(R)) for R in sol.Rt])'
	X = reduce(hcat, sol.xt)'
	plot(X, rib=σ, layout=5)
	plot!([0,500,500,1000], [0.03, 0.03, 0.06, 0.06], l=(:dash, :white), sp=5, lab="\$a_1\$")
end
...
manifest_format = "2.0"
project_hash = "a836e2692268889ecaf7b51509ab59181c42fa61"
...
deps = ["Distributions", "ForwardDiff", "Lazy", "LinearAlgebra", "LoopVectorization", "NonlinearSolve", "PDMats", "Parameters", "Polyester", "Printf", "Random", "RecipesBase", "Requires", "SciMLBase", "SeeToDee", "SimpleNonlinearSolve", "StaticArrays", "Statistics", "StatsAPI", "StatsBase", "SymbolicIndexingInterface"]
git-tree-sha1 = "00706a7b68583e96e6af9e458b4393f3efd50f9f"
...
[[deps.SeeToDee]]
deps = ["FastGaussQuadrature", "ForwardDiff", "LinearAlgebra", "PreallocationTools", "SimpleNonlinearSolve", "StaticArrays"]
git-tree-sha1 = "71f843c20963021452d9c6ed829115620da17176"
