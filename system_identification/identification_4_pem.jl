...
# ╔═╡ 6dfa7f3a-55e8-11ee-119f-db210c4b79c2
using ControlSystemIdentification, ControlSystemsBase, Plots, Statistics, LinearAlgebra, Random, PlutoUI

# ╔═╡ 09b173f4-87eb-4c48-a653-bd1bb73f10ca
md"""
...
\varepsilon &= y_{d} - Cx - Du
\end{align}
```

```math
\operatorname{minimize} \sum_i \varepsilon_i^T \varepsilon_i \quad\text{w.r.t.}\quad {A,B,C,D,K,x_0}
```
...
# ╔═╡ 777a9c8b-7f91-42c2-8ceb-312486088d5d
gr(dpi=1000);

# ╔═╡ e72db47c-e921-48ff-8f18-67274ea5707a
md"# Experimental data"

# ╔═╡ 9ca0d291-c730-4310-a209-fadd8a1fb369
md"""
![DMM](https://help.juliahub.com/juliasimcontrol/dev/figs/double_mass.png)
"""

# ╔═╡ 789b83f6-89a8-4c67-8640-a67cc20f3def
md"""
# Non-parametric estimate
using FFT
```math
G(i\omega) \approx \dfrac{Y(i\omega)}{U(i\omega)}
```
"""

# ╔═╡ ffa3711d-d3e1-4370-89ef-8e68de4248db
md"""
# Prediction-error estimate

Fit two models of order 4: Focus on *prediction* or *simulation*
"""

# ╔═╡ d680d4d5-459c-43f5-8466-ae9c7c969298
md"Evaluate using simulation and prediction"

# ╔═╡ 997918f4-0695-4d4d-83c8-22e4f5b8418f
md"""Prediction horizon: ``h`` = $(@bind h Slider(1:20, show_value=true, default=5))"""

# ╔═╡ e9a262d3-dc38-4801-85d3-04083c827252
md"""
## Comparison to the true (unknown) model
- Fit is good in frequency range of the chirp
- Phase is not quite accurate!
"""

# ╔═╡ 0685c390-1f81-4c14-ae46-77853b5a2ea6
md"""
The importance of *good pahse margins* is evident from the Nyquist plot.
"""

# ╔═╡ 05ddaa40-9fc9-46f9-bce8-d0ff26af1d47
md"""
Learned simulation models may be unstable
"""

# ╔═╡ 96c49fd0-b799-4fd9-b787-de088f439250
md"""
Learned predictor is stable:
"""

# ╔═╡ 1f50c183-ebfc-422e-a3c3-1cd409f6680e
md"""
## Details on the parametrization

Tridiagonal ``A`` and full ``B, C, K``.
"""

# ╔═╡ 5f961c69-5ee6-4312-87bc-bf83bf972777
md"""
Transform to the basis of the true model (difficult to do in practice, but possible with simulated data)
"""

# ╔═╡ 8f147738-ad0d-4ec1-9b68-0a8a3fbbcdb4
md"""
## The true (unknown) system model 
"""

# ╔═╡ b403b12c-4d1c-4187-88d3-c6a36f10c43f
P = DemoSystems.double_mass_model()

# ╔═╡ cfc4685a-b206-4d67-bd90-7eab578bd25e
Ts = 0.01

# ╔═╡ ef42bf0d-e668-488b-b01f-2e4fea22a94f
Pd = c2d(P, Ts);

# ╔═╡ 60487cd8-d38c-45e5-aeb1-c276c3e0c5ef
bodeplot(P, hz=true)

# ╔═╡ 495ef8eb-219c-4274-b9b0-3ea38c441f09
...
# ╔═╡ c3991c80-a9bd-43f4-a4a7-8538a831a35c
begin
	f0 = 1
	f1 = 7
	u = chirp(Ts, f0, f1, 20)

	# This code corresponds to performing an experiment
	res = lsim(Pd, u')
	function friction!(d)
		for i in axes(d.u, 2)
			v = d.x[2, i]
...
# ╔═╡ 2d669dde-21ca-42c1-b868-6007817e14a4
d = iddata(res)

# ╔═╡ ab1e85c2-c587-43df-9f30-c3dc5d0e1a9c
modelp, x0p, optresp = newpem(d, 4, show_trace=false, stable=false)

# ╔═╡ 55e2a4a3-6716-45a3-9175-9f1a50fa4ec8
predictor = observer_predictor(modelp);

# ╔═╡ 88f16bca-090c-4bdb-b961-13a5812158e4
pzmap(d2c(predictor))

# ╔═╡ 56b0c50e-e51d-429d-9cd3-a9bdb0def538
modelp

# ╔═╡ 8bede8e4-ee90-43a6-ab98-eac1add935c2
# ╠═╡ show_logs = false
T = find_similarity_transform(d2c(modelp), P)

# ╔═╡ a3676779-2e66-4106-89df-fa9489dd2650
# ╠═╡ show_logs = false
begin
	modelT = similarity_transform(d2c(modelp), T)
	function trunc_zero!(A)
		A[abs.(A) .< 100eps(maximum(abs, A))] .= 0
		A .= round.(A, digits=5)
	end
	trunc_zero!(modelT.A); trunc_zero!(modelT.B); trunc_zero!(modelT.C)
	modelT
end

# ╔═╡ 0bb07ee5-fe23-4a79-82ba-2332e8d83974
models, x0s, optress = newpem(d, 4, focus=:simulation, show_trace=true, stable=false)

# ╔═╡ 3cf2027a-42d9-4d43-bb38-efbe2edb01f0
nyquistplot(30 .* [Pd, modelp, models], lab=["True" "Pred" "Sim" ""], ylims=(-5, 1), xlims=(-3, 3), unit_circle=true)

# ╔═╡ 840b4fa8-b9cc-4b60-a98e-6cbe0d71d705
# ╠═╡ show_logs = false
begin
	pzmap(P, c=:blue, lab="True model")
	pzmap!(d2c(modelp), c=:orange, markersize=7, lab="Pred")
	pzmap!(d2c(models), c=:green, markersize=5, markerstrokewidth=3, lab="Sim", legend=true, xlims=(-2, 0.5))
end

# ╔═╡ d521fc5f-3600-48c4-918b-8d24473e873f
simplot(modelp, d, x0p[end-3:end], sysname="Pred"); simplot!(models, d, x0s[end-3:end], sysname="Sim", ploty=false)

# ╔═╡ 86192252-3626-4cd6-9435-40ffe4d28dee
predplot(modelp, d; h, sysname="Pred")

# ╔═╡ e7a7f395-78dc-4b02-afbf-50b2bb0fdd60
plot(tfest(d), hz=true); vline!([f0 f1], lab=["f0" "f1"])

# ╔═╡ bba77116-6964-477e-9348-8151e824ad42
bodeplot([Pd, modelp, models], lab=["True" "" "Pred" "" "Sim" ""], hz=true); vline!([f0 f1], lab=["f0" "f1"], sp=1, legend=:bottomleft)

# ╔═╡ 54b8b5fa-cf0c-48ac-a7e5-9009ca222cef
md"""
# Advanced options
"""

# ╔═╡ f7ca6059-723d-4992-9688-1b435e9890c6
newpem
...
[compat]
ControlSystemIdentification = "~2.6.2"
ControlSystemsBase = "~1.8.0"
Plots = "~1.38.17"
PlutoUI = "~0.7.52"
"""
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "df16f80f96b962dba68816b6156c25d589979c25"
