...
# ╔═╡ 54ed402a-53ba-11ee-055b-63504b4de7cc
using ControlSystemIdentification, ControlSystemsBase, Plots, Statistics, LinearAlgebra, Random

# ╔═╡ 4ae26585-5505-434e-a9ea-0deba0202c66
begin
	using Symbolics
	@variables y[1:10] u[1:10]
	y = collect(y)
	u = collect(u)
end;

# ╔═╡ 6ce9e6f5-d5cc-4bef-9c30-aaf7442a5d77
gr(); theme(:dark)

# ╔═╡ 837b73f1-1c31-47c8-a6a0-ddcd2e763737
md"""
# Subspace-based identification

"PCA for data from linear dynamical systems"
...
- Form large Hankel matrices of past and future inputs and outputs
- Perform SVD (and some other factorizations)
- Figure out ``A,B,C,D,K``
- Only linear algebra!
- Many flavors exist (N4SID, MOESP, CVA, IVM, ...)
- Dynamic Mode Decomposition (DMD) but for *real data*
"""

# ╔═╡ 77841a19-7f8d-4847-b7b8-786f3e09311d
md"""
Hankel matrix:
"""

# ╔═╡ 94363eb2-d379-47ef-857b-929649c8e8b8
[ControlSystemIdentification.hankel(y, 3) ControlSystemIdentification.hankel(u, 3)]

# ╔═╡ a44438d1-163b-4572-b786-084dfc9cedbf
md"The identification data is very noisy"

# ╔═╡ df335f84-3e22-42ed-9f80-903d650d14a6
md"Plot the first few seconds of the data"

# ╔═╡ 2ab5c436-835f-4f03-b2ed-5cb6707eeef1
md"If the data comes form a linear system, the Hankel matrix should have low rank"

# ╔═╡ 36c052da-82e7-4f23-b6a7-8ed1e0ea5d2f
md"""
# Subspace vs. PEM
- Subspace-based id. is non-iterative, does not need initial guess 👍
- Subspace-based id. is very fast 👍
- Subspace-based id. is *typically* biased for closed-loop data 👎
- PEM often results in better prediction models with small data

`subspaceid` is used to initialize `newpem`
"""

# ╔═╡ 9afd8b4c-670a-44a5-8a10-97c3ba81e731
md"""
# Generate data
"""

# ╔═╡ ff5cab42-2a87-49e1-b9b7-9bfee3e23bbb
G = feedback(delay(1.5)*tf(1, [1, 1]));

# ╔═╡ a4c5a8ab-0aac-4155-b0bf-2653897f5b52
Gd = c2d(G, 0.02);

# ╔═╡ 4e42a8b3-b495-4823-b4eb-aea38e948e9d
Gd.nx

# ╔═╡ bfa17d60-d8c0-4a56-baab-2f2a38ed107d
begin
	ui = repeat(randn(1000), inner=15)'
	ui[end÷2 : end] .+= 2
	res = lsim(Gd, ui)
	d0 = deepcopy(iddata(res))
	res.y .+= 0.3 .* randn.()
	d = iddata(res)
end

# ╔═╡ 97837d9d-9834-44ff-a0c0-0819ff9696e4
d

# ╔═╡ ff14e839-2beb-4557-97b7-dd4ca462c4eb
plot(d, lab="Identification data"); plot!(d0, lab="Noise-free data")

# ╔═╡ 27cebd0b-3f93-4282-b9dc-c87e6816aac2
plot(d[1:400], lab="Identification data"); plot!(d0[1:400], lab="Noise-free data")

# ╔═╡ 0866d60d-4afd-4f18-bc20-40f2c13639b1
welchplot(d)

# ╔═╡ f6e6a43f-d368-4d2e-a85d-40b989781d7d
coherenceplot(d); #coherenceplot!(d, (; method=:corr, σ=0.03), alpha=0.5)

# ╔═╡ 411c9cb3-9ae9-4840-b343-216e002230e7
Ĝ, N̂ = tfest(d)

# ╔═╡ f5b196d5-a84f-4cd1-bdb8-7e6191a57857
plot(Ĝ, legend=:bottomleft); twinfig = twinx(); plot!(twinfig, coherence(d), c=:blue, ylabel="Coherence", alpha = 0.6, xlabel="")

# ╔═╡ 93b649d0-43a4-474e-8cfd-1d402c338435
model = subspaceid(d, :auto, r=400, verbose=true)

# ╔═╡ d0911ecc-2cf9-46a9-99bd-a8b9586d9912
scatter(model.s.S, title="Singular value spectrum", yscale=:log10)

# ╔═╡ 0efe9d76-3825-47d5-9ef4-af5348f498fb
residualplot(model, d)

# ╔═╡ 4a31f2ff-18eb-4ca6-84c9-fe26a9c9b6e6
model2 = subspaceid(d, 30, r=400)

# ╔═╡ abd38a0e-8d88-415e-9161-63f65bbf9e33
begin
	ω = exp10.(-1.6:0.01:1.6)
	bodeplot(G, ω, lab="True")
	bodeplot!(model.sys, ω, lab="Model auto order $(model.nx)")
	bodeplot!(model2.sys, ω, lab="Model order $(model2.nx)", legend=:bottomleft)
end

# ╔═╡ c5ea0728-4700-41db-b81f-d013d0600aaf
residualplot(model2, d, -100:300)

# ╔═╡ 6981bfaf-0be3-4265-a63f-2b83433a411f
simplot(model2, d, zeros(model2.nx))
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "cda26f0de804e4673d211616de6f9a519b092255"
